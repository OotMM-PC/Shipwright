...
void SaveManager::InitFileNormal() {
    ...
    for (int flag = 0; flag < ARRAY_COUNT(gSaveContext.ship.shipInf); flag++) {
        gSaveContext.ship.shipInf[flag] = 0;
    }
...
void SaveManager::LoadBaseVersion1() {
    ...
    const char* shipInfKey = Instance->currentJsonContext->contains("shipInf") ? "shipInf" : "randomizerInf";
    SaveManager::Instance->LoadArray(shipInfKey, ARRAY_COUNT(gSaveContext.ship.shipInf), [](size_t i) {
        SaveManager::Instance->LoadData("", gSaveContext.ship.shipInf[i]);
    });
}
...
void SaveManager::SaveBase(SaveContext* saveContext, int sectionID, bool fullSave) {
    ...
    SaveManager::Instance->SaveArray("shipInf", ARRAY_COUNT(saveContext->ship.shipInf), [&](size_t i) {
        SaveManager::Instance->SaveData("", saveContext->ship.shipInf[i]);
    });

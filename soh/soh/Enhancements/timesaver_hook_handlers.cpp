...
bool ForcedDialogIsDisabled(ForcedDialogMode type) {
    return (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipForcedDialog"), FORCED_DIALOG_SKIP_NONE) & type) != 0;
}
...
void TimeSaverOnVanillaBehaviorHandler(GIVanillaBehavior id, bool* should, va_list originalArgs) {
    ...
    switch (id) {
        ...
        case VB_PLAY_ENTRANCE_CS: {
            ...
            // set)
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Entrances"), 0) &&
        ...
        case VB_PLAY_ONEPOINT_CS: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.OnePoint"), 0)) {
        ...
        case VB_SHOW_TITLE_CARD:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.DisableTitleCard"), 0)) {
                *should = false;
            }
            break;
        case VB_WONDER_TALK: {
        ...
        case VB_OWL_INTERACTION: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipOwlInteractions"), 0) && *should) {
        ...
        case VB_DAMPE_GRAVEDIGGING_GRAND_PRIZE_BE_HEART_PIECE:
            if (CVarGetInteger(CVAR_ENHANCEMENT("GravediggingTourFix"), 0)) {
        ...
        case VB_PHANTOM_GANON_DEATH_SCENE: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.QuickBossDeaths"), IS_BOSS_RUSH)) {
                *should = false;
        ...
        case VB_NABOORU_KNUCKLE_DEATH_SCENE: {
            ...
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.QuickBossDeaths"), 0)) {
...
void TimeSaverOnActorInitHandler(void* actorRef) {
    ...
    if (actor->id == ACTOR_EN_DNT_DEMO && CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), 0)) {
...
void TimeSaverOnSceneInitHandler(int16_t sceneNum) {
    switch (sceneNum) {
        ...
        case SCENE_DEKU_TREE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), 0)) {
...
static void TimeSaverQueueItem(GetItemID getItemId) {
    vanillaQueuedItemEntry = ItemTableManager::Instance->RetrieveItemEntry(MOD_NONE, getItemId);
}
...
static void TimeSaverRegisterHooks() {
    ...
    COND_HOOK(OnItemReceive, true, TimeSaverOnItemReceiveHandler);
    COND_HOOK(OnPlayerUpdate, true, TimeSaverOnPlayerUpdateHandler);
    COND_HOOK(OnFlagSet, CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), 0),
